...
export function TopicCloud({
  ...
  return (
    <Card padding="sm" interactive className="problems-subsection-shell space-y-2 rounded-[26px] p-3 md:p-4">
      <div className="flex flex-col gap-1.5 md:flex-row md:items-end md:justify-between">
        <div className="space-y-0.5">
          ...
          <div className="space-y-0">
            <h2 className={`text-[1rem] font-semibold tracking-tight text-pebble-text-primary md:text-[1.04rem] ${isUrdu ? 'rtlText' : ''}`}>{title}</h2>
            <p className={`max-w-[58ch] text-[12.5px] leading-[1.55] text-pebble-text-secondary ${isUrdu ? 'rtlText' : ''}`}>{subtitle}</p>
          </div>
        </div>
        <div className="pebble-chip rounded-full px-2 py-0.5 text-[10px] font-medium">
      ...
      <div className="flex flex-wrap gap-1">
        {visibleTopics.map((entry) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-medium transition-all duration-150 ease-out hover:-translate-y-[0.5px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pebble-accent/45 ${selected
                  ? isDark
        ...
        {hiddenCount > 0 ? (
          <button
            ...
            className={`pebble-control inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium transition-all duration-150 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pebble-accent/45 ${
              isDark
                ? 'text-[hsl(220_16%_84%)]'
                : 'text-[hsl(221_24%_36%)]'
            }`}
