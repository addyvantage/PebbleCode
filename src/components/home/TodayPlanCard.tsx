...
function getFocusLoadLabel(state: FocusLoadState, stateCopy: Record<string, string>) {
    if (state === 'balanced') return stateCopy.stateBalanced ?? ''
    if (state === 'stretch') return stateCopy.stateStretch ?? ''
    if (state === 'heavy') return stateCopy.stateHeavy ?? ''
    return stateCopy.stateNeutral ?? ''
}
...
function buildExpectedOutcome(params: {
    ...
    if (streak === 0 && mentionsWarmup) {
        return outcomes.backInRhythm ?? ''
    }

    if (mentionsReview && mentionsWarmup) {
        return outcomes.reviewAndWin ?? ''
    }

    if (loadState === 'heavy') {
        return outcomes.heavy ?? ''
    }

    if (mentionsDrill && plannedMinutes <= FOCUS_TARGET_MINUTES) {
        return outcomes.drill ?? ''
    }
                ...
                <div className="mb-5 flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${loadChipClass}`}>
                            ...
                            <span>{`${planCopy.focusLoad ?? ''} ${plannedMinutes}/${FOCUS_TARGET_MINUTES}m`}</span>
                            {plan ? (
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between border-b border-pebble-border/10 px-5 py-4 bg-pebble-overlay/[0.02] relative z-10">
                            ...
                            <button
                                ...
                                aria-label={planCopy.closeDialog ?? t('actions.close')}
                            >
                                <X className="h-4 w-4" />
                            </button>
                        </div>
