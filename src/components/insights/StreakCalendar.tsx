  ...
  return (
    <div className="space-y-3 rounded-2xl border border-pebble-border/30 bg-pebble-overlay/[0.05] p-4">
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className={`text-base font-semibold text-pebble-text-primary ${isRTL ? 'rtlText' : ''}`}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-1.5">
          <span className="inline-flex items-center gap-1 rounded-full border border-pebble-warning/35 bg-pebble-warning/12 px-2 py-1 text-xs text-pebble-warning">
            <Flame className="h-3.5 w-3.5" aria-hidden="true" />
            <span className="ltrSafe font-semibold">{streak}</span>
          </span>
          <span className="inline-flex items-center gap-1 rounded-full border border-pebble-border/35 bg-pebble-overlay/[0.08] px-2 py-1 text-xs text-pebble-text-secondary">
          ...
          {isTodayComplete ? (
            <span className="inline-flex items-center gap-1 rounded-full border border-pebble-success/35 bg-pebble-success/12 px-2 py-1 text-xs text-pebble-success">
      ...
      <div className="space-y-2 rounded-xl border border-pebble-border/25 bg-pebble-canvas/55 p-3">
        <div className="flex items-center justify-between gap-2">
          <button
            type="button"
            onClick={() => setCursor((prev) => new Date(prev.getFullYear(), prev.getMonth() - 1, 1))}
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-pebble-border/35 bg-pebble-overlay/[0.08] text-pebble-text-secondary transition hover:bg-pebble-overlay/[0.14] hover:text-pebble-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pebble-accent/45"
            aria-label={t('insights.calendar.prevMonth')}
          >
            <ChevronLeft className="h-4 w-4" aria-hidden="true" />
          </button>
          <p className={`text-sm font-medium text-pebble-text-primary ${isRTL ? 'rtlText' : ''}`}>
            {monthLabel}
          </p>
          <button
            type="button"
        ...
        <div className="grid grid-cols-7 gap-1">
          {monthGrid.weeks.flat().map((day) => (
            <div
              ...
              className={`relative flex aspect-square items-center justify-center rounded-md border text-xs transition ${
                day.isComplete
                  ? 'border-pebble-success/35 bg-pebble-success/18 text-pebble-success'
            ...
            >
              ...
              {day.isComplete ? (
                <Check className="pointer-events-none absolute bottom-0.5 right-0.5 h-3 w-3 text-pebble-success" aria-hidden="true" />
              ) : null}
            </div>
          ))}
        </div>
